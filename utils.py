...
class Storage:
    ...
    def _pop_file(self, hashed_files: queue.Queue[File | object], ) -> Iterable[File | object]:
        """Use locally only"""
        while True:
            try:
    ...
    def __init__(self,
        ...
        assert dirs_counter.get() == 0
        assert files_counter.get() == 0
    ...
    def run(self, hashed_files: queue.Queue[File|object]) -> None:
        ...
        hashed_files_counter = Counter()  # not access. from outside

        for file in self._pop_file(hashed_files):
            if not file.marks_empty_dir():
                self.handle_file(file)
                hashed_files_counter.incr()
            else:
                # handle the empty dir the File obj links to
                self.update_dir(file.parent)

        num_files_processed = hashed_files_counter.get()  # flushed
        logger.info('Processed %d queued files', num_files_processed)

        self.dirs_counter.flush()
        self.files_counter.flush()

        # num_files_stored.flush()
        # num_dirs_stored.flush()

        files_stored = self.files_counter.get()  # flushed
        dirs_stored = self.dirs_counter.get()  # flushed

        logger.info('Stored %d files and %d directories',
                    files_stored, dirs_stored)

        assert files_stored == num_files_processed
